...
no console:
*/
console.log( 'Adicionando seu nome no texto: ' + text.replace("Manuel Marques de Sousa", "Geovana Silva Ribeiro") );
// ?
...
*/
console.log( '\nTrocando naturalidade: ' +  text.replace("brasileiro", "paulista"));
// ?
...
*/
console.log( '\nTrocando números por -: ' + text.replace(/\d/g, "-") );
// ?
...
*/
console.log( '\nTrocando de "D" a "h" por "0": ' + text.replace(/[d-h]/gi, "0") );
// ?
...
*/
console.log( '\n"O Centauro de Luvas" em caixa alta: ' + text.replace(/(O Centauro de Luvas)/gi, function( capturaTotal, frase ){
...
function getMonthNumber( monthName ){
    ...
    //return `O mês de ${monthName} é representado pelo número ${monthNumber}`;
}

console.log( getMonthNumber("Fevereiro") );
console.log( getMonthNumber("Junho") );
...
var regexDate = /(\d{2})\s(de)\s(\w*)\s(de)\s(\d{4})/g;

console.log( '\nRegex que vai fazer o match com as datas do texto:' , text.replace(regexDate, function( allCaptures ){
    return allCaptures;
}) );
...
function replaceDate(text){
    text.replace(regexDate, function( allCaptures, day, separator, month, separator2, year ){
